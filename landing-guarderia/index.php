...
<head>
	...
	<script src="../panel/assets/js/jquery.js"></script>
...
<script type="text/javascript" src="https://www.kmimos.com.mx/wp-content/plugins/kmimos/subscribe/includes/js/script.js?ver=4.4.7"></script>
	 <script type='text/javascript'>
            ...
            function register(){     
                if( $('#fieldEmail').val() == ""){
                    $("#msg-vacio").removeClass('span-email-hide');
                    $("#msg-vacio").addClass('span-email-show');
                    return false;
                }else{
                    var mail= $('#fieldEmail').val();
                    var datos = {'source': 'landing-guarderia', 'email': mail}
                    var result = getGlobalData("../../../landing/newsletter.php?source=landing-guarderia&email="+mail,'GET', null);
                        console.log(result);
                    if (result == 1) {
