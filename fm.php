...
$default_timezone = 'America/Caracas'; // UTC+3
...
if (isset($_GET['edit'])) {
    ...
    function vlz_en($text){
        return htmlspecialchars($text, ENT_QUOTES, 'UTF-8');
    }

    function get_home_url( $query = true ){
        $base = str_replace("?".$_SERVER["QUERY_STRING"], "", $_SERVER["REQUEST_URI"]);
        $base = $_SERVER["REQUEST_SCHEME"]."://".$_SERVER["HTTP_HOST"].$base;
        return ( $query ) ? $base."?".$_SERVER["QUERY_STRING"] : $base ;
    }

    if( isset($_POST["edit"]) ){
        $ruta = dirname(__FILE__)."/".$_POST["path"];
        $fp = fopen($ruta, "w");
        $fwrite = fwrite($fp, $_POST["txt"]);
        fclose($fp);
    ...
    $back = explode("/", str_replace("edit&path=", "", $_SERVER["QUERY_STRING"]));
    if ( count($back) == 1 ) {
        $back = '';
    }else{
    ...
    <div class="path">
        <p> 
            <b><a href="<?= get_home_url(false).'?p='.$back ?>"> Atras </a></b> --- 
            <b>Editar Archivo</b>
        </p>
        <?php
            $txt = '';
            $ruta = dirname(__FILE__)."/".$_GET["path"];
            $file = fopen($ruta, "r");
            $cont = 0;
            while(!feof($file)){
                $txt .= fgets($file); $cont++;
        ...
        ?>
        <style type="text/css">
            .submit_edit { position: fixed; right: 20px; bottom: 20px; width: 50px; height: 50px; border-radius: 50%; border: solid 1px #CCC; cursor: pointer; }
            .submit_edit:hover { background: #EEE; }
        </style>
        <form action="?edit&path=<?= vlz_en($_GET["path"]) ?>" method="post" enctype="multipart/form-data">
            <input type="hidden" name="edit" value="">
            <input type="hidden" name="path" value="<?php echo fm_enc($_GET["path"]) ?>">
            <textarea name="txt" style="width: calc( 100% - 7px ) ; height: calc( 100vh - 130px ); white-space: nowrap;"><?= $txt ?></textarea>
            <input class="submit_edit" type="submit" value="Ok" />
        </form>
    </div>
    <?php
    fm_show_footer();
...
<tr>
<td><label><input type="checkbox" name="file[]" value="<?php echo fm_enc($f) ?>"></label></td>
<td><div class="filename"> <a title="Editar [ <?= FM_PATH."/".$f ?> ]" href="?edit&path=<?= FM_PATH."/".$f ?>"> Edit </a> >  <a href="<?php echo fm_enc($filelink) ?>" title="File info"><i class="<?php echo $img ?>"></i> <?php echo fm_enc(fm_convert_win($f)) ?></a><?php echo ($is_link ? ' &rarr; <i>' . fm_enc(readlink($path . '/' . $f)) . '</i>' : '') ?></div></td>
