<div>
    <span style="color: #940d99; font-weight: 600;">SERVICIO:</span> <strong style="text-transform: uppercase;">[tipo_servicio]</strong>
</div>
    
<table cellpadding="0" cellspacing="0" style='padding: 15px 0px 20px; overflow: hidden; font-family: Arial; font-size: 12px; letter-spacing: 0.3px; color: #000000;'>
    
    <tr style=''>
        <td style='height: 20px; text-align: center; vertical-align: middle; width: 50px;'>
          <img style="padding: 3px 0px;" src='[URL_IMGS]/icosNews/min_calendar.png'  > 
        </td>
        <td style='height: 20px; vertical-align: middle; font-size: 17px;'> 
            Del [inicio] al [fin] del [anio]
        </td>
    </tr>
    
    <tr style=''>
        <td style='height: 20px; text-align: center; vertical-align: middle; width: 50px;'>
          <img style="padding: 3px 0px;" src='[URL_IMGS]/icosNews/min_clock.png'  > 
        </td>
        <td style='height: 20px; vertical-align: middle; font-size: 17px;'> 
            [tiempo]
        </td>
    </tr>
    
    <tr style=''>
        <td style='height: 20px; text-align: center; vertical-align: middle; width: 50px;'>
          <img style="padding: 3px 0px;" src='[URL_IMGS]/icosNews/min_cash.png'  > 
        </td>
        <td style='height: 20px; vertical-align: middle; font-size: 17px;'> 
            Pago por [tipo_pago]
        </td>
    </tr>
                                                    
</table>

<div style="display: [DISPLAY_PASEOS];">
    <div>
        <span style="color: #940d99; font-weight: 600;">INFORMACIÓN DE LOS PASEOS:</span>
    </div>
        
    <table cellpadding="0" cellspacing="0" style='padding: 15px 0px 20px; overflow: hidden; font-family: Arial; font-size: 12px; letter-spacing: 0.3px; color: #000000;'>
        
        <tr style=''>
            <td style='font-weight: 800; padding-right: 10px; height: 20px; text-align: left; vertical-align: middle; width: 50px;'>
              PAQUETE
            </td>
            <td style='height: 20px; vertical-align: middle; font-size: 17px;'> 
                [PAQUETE]
            </td>
        </tr>
        
        <tr style=''>
            <td style='font-weight: 800; padding-right: 10px; height: 20px; text-align: left; vertical-align: middle; width: 50px;'>
              DÍAS
            </td>
            <td style='height: 20px; vertical-align: middle; font-size: 17px;'> 
                [DIAS]
            </td>
        </tr>
                                                        
    </table>
</div>