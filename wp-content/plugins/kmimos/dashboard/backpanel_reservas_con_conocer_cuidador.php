<?php global $wpdb;
// Reservas 
require_once('core/ControllerReservasConConocerCuidador.php');
...
<div class="row">
	<div class="col-md-12 col-sm-12 col-xs-12">
		...
		<div class="row text-right"> 
			<div class="col-sm-12">
		    	<form class="form-inline" action="<?php echo get_home_url(); ?>/wp-admin/admin.php?page=bp_reservas_conocer" method="POST">
				  <label>Filtrar:</label>
			...
			<table id="tblReservas" class="table table-striped table-bordered dt-responsive table-hover table-responsive nowrap datatable-buttons" 
			  ...
			  <tbody>
			  	...
			  	<?php foreach( $reservas as $reserva ){ ?>
					...
					$f_ini = date('Y-m-d', strtotime( '- 5 days', strtotime( $reserva->fecha_solicitud)) );
					$f_fin = $reserva->fecha_solicitud ;
				  	$conocer = getConocer( $f_ini, $f_fin, $reserva->cliente_id, $reserva->cuidador_id );
					if( !empty($conocer) ){
						$nro_solicitud = $conocer['rows'][0]['Nro_solicitud'];
						switch($conocer['rows'][0]['Estatus']){
							case 'publish':
								$estatus_solicitud = 'Publicado';
								break;
							case 'draft':
								$estatus_solicitud = 'Cancelado';
								break;
							default:
								$estatus_solicitud = $conocer['rows'][0]['Estatus'];
								break;
				    	...
				    	<th class="text-center"><?php print_r($nro_solicitud); ?></th>
				    	<th class="text-center"><?php print_r($estatus_solicitud); ?></th>
